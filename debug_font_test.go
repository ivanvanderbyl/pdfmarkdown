...
func TestDebug_SamplePDF_FontSizes(t *testing.T) {
	...
	fmt.Printf("Font Size Distribution (top 10):\n")
	...
	if headingCandidates == 0 {
		fmt.Printf("  (none found - all text is the same size)\n")
	}
	fmt.Printf("\n")

	// Show some sample single-line paragraphs
	fmt.Printf("Sample Single-Line Paragraphs:\n")
	sampleCount := 0
	for i := 0; i < pageCount.PageCount && sampleCount < 5; i++ {
		pageResp, err := instance.FPDF_LoadPage(&requests.FPDF_LoadPage{
			Document: doc.Document,
			Index:    i,
		})
		...
		for _, para := range page.Paragraphs {
			if len(para.Lines) == 1 && sampleCount < 5 {
				line := para.Lines[0]
				...
				fmt.Printf("  [Page %d] %.2f pt (%.2fx median) %s: %q\n",
					i+1, maxFontSize, maxFontSize/median,
	...
	fmt.Printf("\nFont names:\n")
	type fontFreq struct {
		name  string
		count int
	}
	var fonts []fontFreq
	for name, count := range fontNames {
		fonts = append(fonts, fontFreq{name, count})
	}
	sort.Slice(fonts, func(i, j int) bool {
		return fonts[i].count > fonts[j].count
	})
	for _, f := range fonts {
		percentage := float64(f.count) / float64(len(allFontSizes)) * 100
		fmt.Printf("  %s: %d words (%.1f%%)\n", f.name, f.count, percentage)
	}

	fmt.Printf("\nStyle summary:\n")
	fmt.Printf("  Bold words: %d\n", boldCount)
	fmt.Printf("  Italic words: %d\n", italicCount)
}
